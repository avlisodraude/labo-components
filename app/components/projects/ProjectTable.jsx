...
class ProjectTable extends React.PureComponent {
  ...
  sortProjects(projects, sort) {
    ...
    switch (sort.field) {
      ...
      case 'bookmarks':
        sorted.sort(
          (a, b) => this.getBookmarkCount(a.id) > this.getBookmarkCount(b.id)
        );
        break;
      case 'owner':
    ...
    return [
      {
        ...
        content: (
          <Link to={'/workspace/projects/' + project.id}>{project.name}</Link>
        )
      },
      {
          ...
          <span>
            ...
            {project.getCollaboratorCount() ? (
              <span className="collaborators">
                {project.getCollaboratorCount()} Collaborator{project.getCollaboratorCount() !==
                1
                  ? 's'
                  : ''}
              </span>
      ...
      },
      {
        content: project.canExport(currentUserId) ? (
          <a
            className="btn blank"
            onClick={exportDataAsJSON.bind(this, project)}
          >
            Export
          </a>
        ) : (
