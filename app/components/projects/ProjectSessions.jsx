...
class ProjectSessions extends React.PureComponent {
  ...
  constructor(props) {
    ...
    // Add dummy data
    const exampleUrl =
      '/tool/exploratory-search?path=/browser/session%3Fid=an-1acf4520-f414-4198-a61f-a91a44fd7408';
  ...
  deleteSession(session) {
    if (
      window.confirm('Are you sure you want to delete session ' + session.name)
    ) {
  ...
  deleteSessions(sessions) {
    if (
      window.confirm(
        'Are you sure you want to delete ' + sessions.length + ' sessions?'
      )
    ) {
      const project = this.props.project;
          ...
          row={session => [
            ...
            { content: session.created.substring(0, 10) },
            {
              content: (
                <a
                  ...
                  onClick={this.deleteSession.bind(this, session)}
                >
                  Delete
                </a>
              )
            },
            {
              content: (
                <a
                  className="btn blank"
                  onClick={exportDataAsJSON.bind(this, session)}
                >
                  Export
                </a>
              )
            },
            {
              content: (
                <a
                  href={session.data.url ? session.data.url : '#no-url-found'}
                  className="btn"
                >
                  Open
                </a>
