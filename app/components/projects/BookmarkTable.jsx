...
class BookmarkTable extends React.PureComponent {
  ...
  reloadData(){
    // filter
    let filtered = this.props.filterItems(this.props.items,this.state.filter);

    // sort
    let sorted = this.props.sortItems(filtered, this.state.order);
          ...
          <div className="filters">
            ...
            <div className="right">
              ...
              <select value={this.state.order} onChange={this.sortChange.bind(this)}>
                {this.props.orders.map((type,index)=>(<option key={index} value={type.value}>{type.name}</option>))}                
              </select>
