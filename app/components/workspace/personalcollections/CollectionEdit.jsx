...
/**
 * Edit the collection as specified by the router, using the CollectionForm component
 */
    ...
    return (
      <div className={IDUtil.cssClassName('project-edit')}>
        <div className="info-bar">
          ...
          <p>
            A user collection contains personal metadata and possibly links to private collection data
          </p>
        </div>
        ...
        <div className={IDUtil.cssClassName('sort-table')}>
          ...
          <table>
              ...
              {this.state.dataList.map((data, i) => {
                  return (
                      <tr key={i}>
                          <td><a href={"/workspace/collections/" + this.props.match.params.id + '/entry/' + data.id}>{data.title}</a></td>
                          <td>{data.descr}</td>
                          <td>{data.dateCreated}</td>
                          <td>{data.creator}</td>
                          <td>{data.fileUrl}</td>
                          <td><button onClick={this.handleDeleteEntry.bind(this,i)}>Delete</button></td>
                      </tr>
                  )
              })}
              <tfoot>
                  <tr>
                      ...
                      <th><button type="button" onClick={this.handleAddEntry} className="primary button add">Add Entry</button></th>
                  </tr>
              </tfoot>
          </table>
        </div>
